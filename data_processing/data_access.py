...
def get_game_data(gs):
    ...
    resourcetype_matrix = np.where(
        resourcetype_matrix == None, 0, resourcetype_matrix)

    for value in resource_dict.keys():
        resourcetype_matrix = np.where(
            resourcetype_matrix == value, resource_dict[value], resourcetype_matrix)

    resourceamount_matrix = np.array([resource.amount
                                      if resource
                                      else None
                                      for resource in resource_matrix.flatten()]).reshape(sqrt, sqrt)
    resourceamount_matrix = np.where(
        resourceamount_matrix == None, 0, resourceamount_matrix)
    ...
    max_val = 32
    padding = [(0, max_val-sqrt), (0, max_val-sqrt)]
    
    padded_data = []
    for i in range(len(data_matrices)):
        padded_data.append(np.pad(data_matrices[i],padding, mode='constant'))
